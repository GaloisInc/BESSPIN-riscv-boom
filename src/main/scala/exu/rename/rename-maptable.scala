  ...
  (implicit p: Parameters) extends BoomModule()(p)
{
  ...
  val bypass_table = Wire(Vec(plWidth, Vec(numLregs, Bool())))
  ...
  for (i <- 0 until numLregs) {
    if (i == 0 && !float) {
      for (j <- 0 until plWidth+1) {
        remap_table(j)(i) := 0.U
        if (j < plWidth) bypass_table(j)(i) := false.B
      }
    } else {
      ...
      val bypass_row = remap_ldst_reqs.map(ldst => ldst(i)).scanLeft(false.B)(_||_)

      for (j <- 0 until plWidth+1) {
        remap_table(j)(i) := remapped_row(j)
        if (j < plWidth) bypass_table(j)(i) := bypass_row(j)
      }
    }
  }
  ...
  for (i <- 0 until plWidth) {
    ...
    val bypass_col = bypass_table(i)
    io.map_resps(i).prs1_was_bypassed := bypass_col(io.ren_uops(i).lrs1)
    io.map_resps(i).prs2_was_bypassed := bypass_col(io.ren_uops(i).lrs2)
    if (float) io.map_resps(i).prs3_was_bypassed := bypass_col(io.ren_uops(i).lrs3) else io.map_resps(i).prs3_was_bypassed := false.B
  }
}
