  ...
  (implicit p: Parameters) extends BoomModule
{
  ...
  for (i <- 0 until plWidth) {
    ...
    io.map_resps(i).prs1 := (0 until i).foldLeft(map_table(io.map_reqs(i).lrs2)) ((p,k) =>
      Mux(io.remap_reqs(k).valid && io.remap_reqs(k).ldst === io.map_reqs(i).lrs2, io.remap_reqs(k).pdst, p))
    io.map_resps(i).prs1 := (0 until i).foldLeft(map_table(io.map_reqs(i).lrs3)) ((p,k) =>
      Mux(io.remap_reqs(k).valid && io.remap_reqs(k).ldst === io.map_reqs(i).lrs3, io.remap_reqs(k).pdst, p))
    io.map_resps(i).prs1 := (0 until i).foldLeft(map_table(io.map_reqs(i).ldst)) ((p,k) =>
