  ...
  (implicit p: Parameters) extends BoomModule()(p)
{
  ...
  val io = IO(new BoomBundle()(p) {
    ...
    val busy_reqs = Input(Vec(plWidth, new MapResp))
  ...
  val busy_table_next = busy_table_wb | (io.ren_uops zip io.rebusy_reqs)
    .map{case (uop, valid) => UIntToOH(uop.pdst) & Fill(numPregs, valid.asUInt)}.reduce(_|_)

  busy_table := busy_table_next

  // Read the busy table.
  for (i <- 0 until plWidth) {
    io.busy_resps(i).prs1_busy := busy_table(io.busy_reqs(i).prs1) || io.busy_reqs(i).prs1_was_bypassed
    io.busy_resps(i).prs2_busy := busy_table(io.busy_reqs(i).prs2) || io.busy_reqs(i).prs2_was_bypassed
    if (float) io.busy_resps(i).prs3_busy := busy_table(io.busy_reqs(i).prs3) || io.busy_reqs(i).prs3_was_bypassed
