  ...
  (implicit p: Parameters) extends BoomModule()(p)
{
  ...
  val io = IO(new BoomBundle()(p) {
    ...
    val can_alloc = Output(Vec(plWidth, Bool()))
  ...
  io.can_alloc := preg_sels.map(_.orR)
  ...
  for (i <- 0 until MAX_BR_COUNT) {
    val list_req = VecInit(io.ren_br_tags.map(tag => UIntToOH(tag.bits)(i))).asUInt
    val new_list = (list_req & br_slots).orR
