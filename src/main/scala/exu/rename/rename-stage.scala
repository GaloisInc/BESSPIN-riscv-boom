...
(implicit p: Parameters) extends BoomModule
{
  ...
  for ((uop, w) <- ren1_uops.zipWithIndex) {
    ...
    uop.pdst := Mux(uop.dst_rtype === RT_FLT,
                  f_preg,
                Mux(uop.ldst =/= 0.U,
                  i_preg,
                  0.U))
  ...
  for ((uop, w) <- ren1_uops.zipWithIndex) {
    ...
    val fmap = if (usingFPU) fmaptable.io.map_resps(w) else Wire(new MapResp)
  ...
  val ren2_imap_resps = Wire(Vec(plWidth, new MapResp))
  val ren2_fmap_resps = Wire(Vec(plWidth, new MapResp))

  if (renameLatency == 2) {
    val r_imap_resps = Reg(Vec(plWidth, new MapResp))
    val r_fmap_resps = Reg(Vec(plWidth, new MapResp))

    when (ren2_will_proceed) {
      r_imap_resps := imaptable.io.map_resps
      r_fmap_resps := fmaptable.io.map_resps
    } .otherwise {
      for (w <- 0 until plWidth) { // TODO This is nasty.
        when (!ren2_will_fire(w)) {
          r_imap_resps(w).prs1_was_bypassed := false.B
          r_imap_resps(w).prs2_was_bypassed := false.B
          r_imap_resps(w).prs3_was_bypassed := false.B

          r_fmap_resps(w).prs1_was_bypassed := false.B
          r_fmap_resps(w).prs2_was_bypassed := false.B
          r_fmap_resps(w).prs3_was_bypassed := false.B
        }
      }
    }

    ren2_imap_resps := r_imap_resps
    ren2_fmap_resps := r_fmap_resps
  } else {
    ren2_imap_resps := imaptable.io.map_resps
    ren2_fmap_resps := fmaptable.io.map_resps
  }
  ...
  for (w <- 0 until plWidth) {
    ...
    io.inst_can_proceed(w) :=
      ren2_will_proceed &&
      ((ren1_uops(w).dst_rtype =/= RT_FIX || ifreelist.io.can_alloc(w)) &&
      (ren1_uops(w).dst_rtype =/= RT_FLT || ffreelist.io.can_alloc(w) || !usingFPU.B) ||
      !ren1_uops(w).ldst_val)
  }
