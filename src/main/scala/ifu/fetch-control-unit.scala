...
//  val f3_bim_target = br_targs(f3_bim_br_idx)
  ...
  val f3_bpd_overrides_bcheck =
    f3_bpd_may_redirect && (!bchecker.io.req.valid || (f3_bpd_redirect_cfiidx < bchecker.io.req_cfi_idx))
  ...
  io.f3_btb_update.valid := RegNext(Mux(f3_bpd_overrides_bcheck,
                                        f3_bpd_btb_update_valid,
                                        bchecker.io.btb_update.valid)) && r_f4_valid
