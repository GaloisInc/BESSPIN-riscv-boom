...
class UpdateEntry(val index_sz: Int)(implicit p: Parameters) extends BoomBundle
{
...
class BrTableUpdate(val index_sz: Int)(implicit p: Parameters) extends BoomBundle
{
  val index      = UInt(index_sz.W)
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val io = IO(new Bundle {
    ...
    val update   = Flipped(Decoupled(new BrTableUpdate(fetchWidth, index_sz)))
  })
...
class PTableDualPorted(
  num_entries: Int
  )(implicit p: Parameters) extends PTable(num_entries)
{
  val p_table = SyncReadMem(num_entries, Vec(fetchWidth, Bool()))
...
class PTableBanked(
  num_entries: Int
  )(implicit p: Parameters) extends PTable(fetchWidth, num_entries)
{
  val p_table_0 = Module(new SeqMem1rwTransformable(num_entries/2, fetchWidth))
  val p_table_1 = Module(new SeqMem1rwTransformable(num_entries/2, fetchWidth))
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val io = IO(new Bundle {
    // Update the h-table.
    val update   = Flipped(Valid(new UpdateEntry(fetchWidth, ptable_idx_sz)))
    // Enqueue an update to the p-table.
    val pwq_enq  = Decoupled(new BrTableUpdate(fetchWidth, ptable_idx_sz))
  })

  val h_table = Module(new SeqMem1rwTransformable(num_h_entries, fetchWidth))
  val hwq = Module(new Queue(new UpdateEntry(fetchWidth, ptable_idx_sz), entries=4))
  ...
  ) extends BoomModule
{
  ...
  val io = IO(new Bundle {
    ...
    val update   = Flipped(Valid(new UpdateEntry(fetchWidth, index_sz)))
  })
  ...
  val p_table = if (dualported) Module(new PTableDualPorted(fetchWidth, num_entries))
                else            Module(new PTableBanked(fetchWidth, num_entries))
  val h_table = Module(new HTable(fetchWidth, num_p_entries = num_entries, share_hbit = share_hbit))

  //------------------------------------------------------------
  // write queue from h-table to p-table.

  val pwq_entries = if (dualported) 2 else 6
  val pwq = Module(new Queue(new BrTableUpdate(fetchWidth, index_sz), entries=pwq_entries))
  ...
  override def toString: String =
    ("  Building (" +
    (num_entries * fetchWidth * 2/8/1024) + " kB) 2-bit counter table for (" +
