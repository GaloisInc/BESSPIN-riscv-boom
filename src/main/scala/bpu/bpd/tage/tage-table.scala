...
class TageTableIo(
  ...
  val tag_sz: Int,
  val cntr_sz: Int,
  val ubit_sz: Int)(implicit p: Parameters)
  extends BoomBundle
{
  ...
  val bp2_resp = Valid(new TageTableResp(tag_sz, cntr_sz, ubit_sz))

  // Write to this table. Can either be an Allocate, Update, or Degrade operation.
  // (Update and Degrade are not mutually exclusive).
  val write = Flipped(Valid(new TageTableWrite(index_sz, tag_sz, cntr_sz, ubit_sz)))
  def WriteEntry(
  ...
  def InitializeIo = {
    ...
    this.write.bits.old := (0.U).asTypeOf(new TageTableEntry(tag_sz, cntr_sz, ubit_sz))
 ...
 * @param ubit_sz size of the usefulness counter in bits
 */
class TageTableResp(val tag_sz: Int, val cntr_sz: Int, val ubit_sz: Int)(implicit p: Parameters) extends BoomBundle
{
  val tag  = UInt(tag_sz.W)
 ...
 * @param ubit_sz size of the usefulness counter in bits
 */
class TageTableEntry(val tag_sz: Int, val cntr_sz: Int, val ubit_sz: Int)(implicit p: Parameters) extends BoomBundle
{
 ...
 * @param ubit_sz size of the usefulness counter in bits
 */
class TageTableWrite(val index_sz: Int, val tag_sz: Int, val cntr_sz: Int, val ubit_sz: Int)(implicit p: Parameters)
  extends BoomBundle
{
  val index = UInt(index_sz.W)
  val old = new TageTableEntry(tag_sz, cntr_sz, ubit_sz)
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val io = IO(new TageTableIo(index_sz, tag_sz, cntr_sz, ubit_sz))
  ...
  // TODO add banking
  val ram = SyncReadMem(num_entries, new TageTableEntry(tag_sz, cntr_sz, ubit_sz))
  ...
  when (io.write.valid || fsm_state === s_clear)
  {
    ...
    val wentry = Wire(new TageTableEntry(tag_sz, cntr_sz, ubit_sz))
