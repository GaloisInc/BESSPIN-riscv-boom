 ...
 * @param num_tables number of tables that can provide a prediction
 * @param table_sizes entries of each table
 * @param history_lengths lengths of the global histories used to index into tables
 * @param tag_sizes size of the tags of each entry in the tables
 ...
 *
 * @param num_tables number of tables that can provide a prediction
 * @param max_history_length max length of global histories
 * @param max_index_sz log2Ceil of the table size
 * @param max_tag_sz max size of the tags in the tables
 * @param cntr_sz size of the prediction counter in bits
 * @param ubit_sz size of the usefulness counter in bits
 */
class TageResp(
  ...
  extends BoomBundle
{
  ...
  val cidxs = Vec(num_tables, UInt(log2Ceil(fetchWidth).W))
...
class TageBrPredictor(
  ...
  extends BoomBrPredictor(
    history_length = history_lengths.max)
{
  ...
  val q_f3_resps = for (i <- 0 until num_tables) yield {
    val q_resp = withReset(reset.toBool || io.fe_clear || io.f4_redirect)
     {Module(new ElasticReg(Valid(new TageTableResp(tag_sizes.max, cntr_sz, ubit_sz))))}
...
//  }
  ...
  for (i <- 0 until num_tables) {
    when (table_wens(i)) {
      ...
      val com_entry = Wire(new TageTableEntry(tag_sizes.max, cntr_sz, ubit_sz))
