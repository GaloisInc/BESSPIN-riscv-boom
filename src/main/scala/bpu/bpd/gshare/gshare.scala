 ...
 */
class GShareResp(val idx_sz: Int)(implicit p: Parameters) extends BoomBundle
{
...
object GShareBrPredictor
{
  def GetRespInfoSize(hlen: Int)(implicit p: Parameters): Int = {
    val dummy = new GShareResp(idx_sz = hlen)
    dummy.getWidth
  }
}
...
class GShareBrPredictor(
   ...
   extends BoomBrPredictor(history_length)
   with HasGShareParameters
{
  ...
  val q_s3_resp = withReset(reset.toBool || io.fe_clear || io.f4_redirect)
    {Module(new ElasticReg(new GShareResp(idx_sz)))}
  ...
  val com_info = (io.commit.bits.info).asTypeOf(new GShareResp(idx_sz))
