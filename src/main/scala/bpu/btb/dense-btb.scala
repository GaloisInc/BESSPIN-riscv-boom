...
class DenseBTB(implicit p: Parameters) extends BoomBTB
{
  ...
  when (blevel === 1.U) {
    ...
    val resp_target = if ((vaddrBits-lsb_sz) <= tag_sz+2*offset_sz+lsb_sz) {
      Cat(data_out(1).offset((vaddrBits-1)-(tag_sz+offset_sz+lsb_sz), 0),
        data_out(0).tag,
        data_out(0).offset,
        0.U(lsb_sz.W))
    } else {
      Cat(s1_pc(vaddrBits-1,tag_sz+2*offset_sz+lsb_sz),
        data_out(1).offset,
        data_out(0).tag,
        data_out(0).offset,
        0.U(lsb_sz.W))
  ...
  if (branch_levels > 2) {
    when (blevel === 2.U) {
      ...
      val resp_target = if ((vaddrBits-lsb_sz) <= 2*tag_sz+3*offset_sz+lsb_sz) {
        Cat(data_out(2).offset((vaddrBits-1)-(2*(tag_sz+offset_sz)+lsb_sz), 0),
          data_out(1).tag,
          data_out(1).offset,
          data_out(0).tag,
          data_out(0).offset,
        ...
        Cat(s1_pc(vaddrBits-1,2*tag_sz+3*offset_sz+lsb_sz),
          data_out(2).offset,
          data_out(1).tag,
          data_out(1).offset,
          data_out(0).tag,
