  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    val dis_ld_val = io.core.dis_uops(w).valid && io.core.dis_uops(w).bits.uses_ldq && !io.core.dis_uops(w).exception
    val dis_st_val = io.core.dis_uops(w).valid && io.core.dis_uops(w).bits.uses_stq && !io.core.dis_uops(w).exception
    when (dis_ld_val)
    {
